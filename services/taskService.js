...
const createTask = async ({ actor, body }) => {
  ...
  fireNotify({
    ...
    selfTitle: `Task created: ${task.title}`,
    selfSubtitle: `You created this ${task.priority} priority task for ${assignedUsers.length} user(s)`,
...
const deleteTask = async ({ actor, id }) => {
  ...
  fireNotify({
    ...
    selfTitle: `Task closed: ${task.title}`,
    selfSubtitle: "You archived this task",
    routeName: "MyTasks",
    payload: { taskId: String(task._id) },
    category: "tasks",
  });
...
const addAssignees = async ({ actor, id, userIds }) => {
  ...
  if (addedIds.length) {
    fireNotify({
      ...
      selfTitle: `Task updated: ${task.title}`,
      selfSubtitle: `You added ${addedIds.length} member(s) to this task`,
      routeName: "TaskDashboard",
      payload: { taskId: String(task._id) },
...
const removeAssignee = async ({ actor, id, userId }) => {
  ...
  fireNotify({
    ...
    selfTitle: `Task updated: ${task.title}`,
    selfSubtitle: `You removed ${entry.userName} from this task`,
    routeName: "MyTasks",
    payload: { taskId: String(task._id) },
    category: "tasks",
  });
...
const sendMessage = async ({ actor, id, body }) => {
  ...
  fireNotify({
    ...
    selfTitle: `You · ${task.title}`,
    selfSubtitle: type === "voice" ? "🎤 You sent a voice note" : `You: ${String(body.text).trim().slice(0, 110)}`,
    routeName: "TaskDashboard",
